...
def new_spec(region, name=None, create_bucket=True, ins_groups=None):
    ...
    with open(_script_dir + "/template.k8s.local.yaml", "r") as f:
        ...
        if ins_groups is None:
            cluster_spec += "\n---\n\n"
            cluster_spec += gen_block("c4.4xlarge", 1, region, name)
        else:
    ...
    if create_bucket:
        s3.create_bucket(get_state_bucket(), region)
...
def create_cluster(ins_groups: dict = None, name=None):
    new_spec(name=name, region=os.environ.get("REGION", "us-west-2"), ins_groups=ins_groups)
...
def main():
    ...
    cmds = {
        # cluster operations
        "up": lambda: create_and_start({"c4.xlarge": 2}),
